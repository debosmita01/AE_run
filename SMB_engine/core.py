...
class MarioGame:
    ...
    def runGame(self, agent, level, timer, marioState = 0, et=False):
        self.setAgent(agent)
        return self._gameLoop(level, timer, marioState, et)

    def _gameLoop(self, level, timer, marioState, et):
        ...
        while self._world.gameStatus == GameStatus.RUNNING:
            if et:
                if self._agent._search_repeat > 1:
                    self._world.gameStatus = GameStatus.TIME_OUT
